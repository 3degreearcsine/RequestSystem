...
def login(request: Request, user_credentials: OAuth2PasswordRequestForm = Depends(),
        ...
        if not expired_token:
            ...
            if not token_exists:
                ...
                return main.templates.TemplateResponse('popup.html', context={'request': request,
                                                                              'not_logged_out': 'Please logout to continue',
    ...
    if not user:
        ...
        return main.templates.TemplateResponse('popup.html', context={'request': request,
                                                                      'error': invalid_username_exception.detail,
                                                                      'url': config.settings.url},
                                               status_code=invalid_username_exception.status_code)
    if not utils.verify(user_credentials.password, user.password):
        database.session.remove()
        invalid_password_exception = HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Invalid Credentials")
        return main.templates.TemplateResponse('popup.html', context={'request': request,
                                                                      'error': invalid_password_exception.detail,
                                                                      'url': config.settings.url},
                                               status_code=invalid_password_exception.status_code)

    access_token = oauth2.create_access_token(data={"user_id": user.id, "user_email": user.email.lower(),
...
def logout(request: Request, token: str = Depends(oauth2.get_user_token),
    ...
    db.refresh(l_out)
    database.session.remove()
    response = main.templates.TemplateResponse('popup.html', context={'request': request, 'detail': "Logout Successful",
                                                                      'url': config.settings.url},
